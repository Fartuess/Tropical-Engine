...
{
	//return length(u_cameraPosition) + 1.0f;
	float AvgDistance = ((Distance0 + Distance1) / 2.0);// * length * length;                                          
	return max(1.0, ((1.0 * mat_tesselationMultiplier) / ((sqrt(AvgDistance)))) * (length) );
	//return max(1.0, min(10.0, 1 / (AvgDistance / mat_tesselationMultiplier - 3.0)));
	/*
    if (AvgDistance <= 2.0) {                                                                   
        return 10.0;                                                                            
    }                                                                                           
