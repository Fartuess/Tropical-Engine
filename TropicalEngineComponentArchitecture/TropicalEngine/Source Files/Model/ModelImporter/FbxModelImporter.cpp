...
namespace TropicalEngine
{
	...
	Model* FbxModelImporter::Load(QString name, class FbxMesh* mesh)
	{
		...
		for (int i = 0; i < Mesh->NumVertex; i++)
		{
			...
			//FbxVector4 tangentHelper = normalMatrix.MultT(fbxTangents[i]);
			//FbxVector4 bitangentHelper = normalMatrix.MultT(fbxBitangents[i]);
			FbxVector4 positionHelper = fbxPositions[i];
			FbxVector4 normalHelper = fbxNormals[i];
			FbxVector4 tangentHelper = fbxTangents[i];
			FbxVector4 bitangentHelper = fbxBitangents[i];
